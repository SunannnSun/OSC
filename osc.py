...
class OperationalSpaceWalkingController(LeafSystem):
    ...
    def CalculateContactJacobian(self, fsm: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        J = self.plant.CalcJacobianTranslationalVelocity(
                    self.plant_context, JacobianWrtVariable.kV, contact_point.frame,
                    contact_point.pt, self.plant.world_frame(), self.plant.world_frame()
                )
    ...
    def SetupAndSolveQP(self,  context: Context) -> Tuple[np.ndarray, MathematicalProgram]:
        ...
        for traj_name in self.trajs:
            ...
            # yddot_cmd_i = yddot_cmd_i.reshape(-1, 1)
            # JdotV_i = JdotV_i.reshape(-1, 1)
            
            # Q_i = 2 * J_i.T @ W_i @ J_i
            # b_i = 2 * J_i.T @ W_i @ JdotV_i - 2 * J_i.T @ W_i @ yddot_cmd_i
            # c_i = JdotV_i.T @ W_i @ JdotV_i + yddot_cmd_i.T @ W_i @ yddot_cmd_i - 2 * JdotV_i.T @ W_i @ yddot_cmd_i
            # prog.AddQuadraticCost(Q_i, b_i, c_i, vdot)

            yddot = (JdotV_i+J_i@(vdot))
            prog.AddQuadraticCost((yddot_cmd_i-yddot).T@(W_i)@(yddot_cmd_i-yddot))
        
        # TODO: Add Quadratic Cost on vdot using self.gains.w_vdot * Identity
        # prog.AddQuadraticCost(self.gains.w_vdot * np.eye(self.plant.num_velocities()), np.zeros(self.plant.num_velocities()), vdot)
        prog.AddQuadraticCost(vdot @ (self.gains.w_vdot * np.eye(self.plant.num_velocities())) @ vdot)
